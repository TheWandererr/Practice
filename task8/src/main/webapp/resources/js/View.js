class View {
    ...
    getPage(posts, add = false, del = false, totalNumber = this._totalNumber) {
        ...
        View.clearInnerHtml(docElement);
        this._displayedNumber = 0;
    ...
    static deleteContentFromAddForm() {
        View.clearInnerHtml(document.getElementById(View._IMAGE_ZONE_ID));
        View._deleteDescriptionContent();
        View._deleteHashTagsContent();
        View._deletePostIdFromAdd();
    }
    ...
    static _loadContentToAddForm(post) {
        // const dropArea = document.getElementById(PageController._IMAGE_ZONE_ID);
        // View.changeDropZoneAppearance(dropArea, post.photoLink);
        View._loadImg(post.photoLink, post.id);
        View._loadDescriptionContent(post.description);
        View._loadHashTagsContent(post.hashTags);
    }
    static _loadImg(what, postId) {
        const where = document.getElementById(View._IMAGE_ZONE_ID);
        const img = document.createElement('img');
        img.setAttribute('id', what);
        img.setAttribute('src', `../../../target/MMA/WEB-INF/uploads/${what}`);
    ...
    _createPostElement(post) {
        return `
                    ...
                    <div class="image"><img src=../../../target/MMA/WEB-INF/uploads/${post.photoLink} alt="/"></div>
